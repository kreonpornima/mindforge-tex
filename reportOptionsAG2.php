        ...
        if (error) {
          ...
          // console.log("in if");
            ...
            if(DrillStructure.length == 0){
              ...
              if(toppivot){
                console.log("Pivot2", toppivot.pivotMode);
              ...
              // const chartRefs = gridApi.getChartRef?.() || [];

              // if (Array.isArray(chartRefs) && chartRefs.length > 0) {
              //   chartRefs.forEach(ref => {
              //     gridApi.destroyChart(ref.chartId);
              //     console.log("Destroyed chart:", ref.chartId);
              //   });
              // } else {
              //   console.log("No charts found to destroy");
              // }

              // 1. Remove existing charts (DOM level)
              setTimeout(() => {
                  document.querySelectorAll('.ag-chart, .ag-charts-container, .ag-popup').forEach(el => {
                      el.remove();
                  });
                  console.log("Existing charts removed");
              }, 10);

              // console.log(data.chartModels)
              //RESTORE CHART
                  ...
                  setTimeout(() => {
                      data.chartModels.forEach(model => {
                          try {
                              ...
                              console.error("Chart restore failed:", err);
                          }
                      });
                  }, 250);
              }
              ...
              if (data.groupDisplayType != '' && data.groupDisplayType != undefined && data.groupDisplayType != null) {
                ...
                gridApi.setGridOption('groupHideOpenParents',data.groupDisplayType === 'multipleColumns' ? true : false);
              }else{
  ...
  <section class="panel report-filters-section">
    <header class="panel-heading" style="display: flex; justify-content: space-between; align-items: center;">
      ...
      <div>
          ...
          <button style="margin-right:40px;" class="btn btn-success btn-xs" onclick="openRequirementModal()">Support Ticket</button>
...
<script>
  ...
  if(DrillStructure == null){
    ...
    gridOptions = {
        ...
        getRowId: params => params.data?.[0]?.toString(), 
        theme: myTheme,
        columnDefs: columnDefs,
        grandTotalRow: 'bottom',
        groupDisplayType: $('#group_display_type').val(),
        // groupDisplayType: 'multipleColumns',
        groupHideOpenParents: $('#group_display_type').val() == 'multipleColumns' ? true : false,      // false → show parent row
        ...
        onFirstDataRendered: params => {
                console.log("First data rendered → autosizing...");
                // params.api.autoSizeAllColumns();
                autoSizeAllColumnsAcrossViewport();
            },
        onGridReady: params => {
  ...
  function setGroupDisplayType(){
    ...
    gridApi.setGridOption('groupHideOpenParents',value === 'multipleColumns' ? true : false);
...
<script>
  async function autoSizeAllColumnsAcrossViewport() {
      if (!gridApi) return;

        const gridDiv = document.getElementById("myGrid");   // <-- update to your grid ID
    const originalWidth = gridDiv.style.width;           // store the original width

    // 1. Temporarily expand width so off-screen columns render
    gridDiv.style.width = "10000px";
    
    // Force a redraw (replacement for doLayout)
    gridApi.refreshCells({ force: true });

    // Wait for browser to render the expanded grid
    await new Promise(resolve => setTimeout(resolve, 30));

    // 2. Auto-size ALL columns (visible + off-screen)
    const allColIds = gridApi.getColumns().map(col => col.getColId());
    gridApi.autoSizeColumns(allColIds);

    console.log("originalWidth", originalWidth)
    // 3. Restore original width or 100%
