...
function isPinnedRowDataCompleted(params) {
  ...
  function loadPivotGridData() {
      ...
      if(allData !== null){
        ...
        fetch("athletes.php?ReportID=<?php echo $ReportID; ?>")
...
// }
  ...
  function UpdatedField(ID, Field, oldValue, newValue, params) {
    ...
    $.ajax({
        url: `Edit_athlete.php`,
        headers: {
            Accept: "application/json",
            "Content-Type": "application/json"
        },
        data: { ID: ID, Field: Field, oldValue: oldValue, newValue: newValue },
        type: "GET",
        success: function(data) {
            data = JSON.parse(data);
            console.log("Got Data from API", data);
            // If the backend returns an error, show the alert
            if (data.success) {
        ...
        error: function(jqXHR, textStatus, errorThrown) {
            console.log("Error from API", jqXHR);
          ...
          if (dataType == "int" || dataType == "bigint" || dataType == "float" || dataType == "decimal" || dataType == "money") {
            ...
            if(editData.length > 0){
              ...
              if (colName === 'sport') {
                  ...
                  fetchSportOptions().then((sports) => {
                      cellEditorParams = {
                          values: sports, // Set dropdown options dynamically
                          valueListMaxHeight: 120, 
                      ...
                      colDefs.push({
                          ...
                          cellEditor: MultiSelectDropdownEditor, // Use custom multi-select editor
                          cellEditorParams: {
                              values: sports, // Set dropdown options dynamically
                          },
                          valueSetter: (params) => {
                      ...
                      // Refresh the grid to apply the new column definition
                      gridApi.setGridOption('columnDefs', colDefs); 
                          ...
                          // Fetch dynamic options
                  // Fetch dynamic options
                  fetchCountryOptions().then((sports) => {
                      cellEditorParams = {
                          values: sports, // Set dropdown options dynamically
                          valueListMaxHeight: 120, 
                          searchEnabled: true, // Enable search bar// Optional: Adjust dropdown height
                      ...
                      colDefs.push({
                          ...
                          valueSetter: (params) => {
                              console.log("In string country section");
                              console.log("Value Setter", params);
                              const newValue = params.newValue;
                              ...
                              if (valueChanged && oldValue != undefined) {
                                if (params.node.rowPinned !== 'top'){
                                  ...
                                  console.log("In country else section");
                                  params.data[params.colDef.field] = newValue; 
                                  return false;
                              }
                          },
