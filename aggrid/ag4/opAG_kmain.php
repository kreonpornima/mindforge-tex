...
function isPinnedRowDataCompleted(params) {
  ...
  // Specify the fields that are mandatory
  const mandatoryFields = ['Athlete', 'Sport', 'Date','country'];

  // Check only the mandatory fields
  return mandatoryFields.every((field) => params.data[field] !== null && params.data[field] !== undefined);
}
        ...
        $.ajax({
            ...
            success: (result) => {
                if (result.success) {
                    ...
                    resolve(result.lastInsertedId); // Resolve with the inserted ID
                } else {
            ...
            error: (xhr, status, error) => {
                ...
                reject(new Error('An error occurred while saving data.')); // Reject with an error
            },
        });
    });
}
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.status === "success") {
                ...
                alert(data.message);
                params.node.setDataValue(Field, oldValue); // Revert the value in the UI
                return false;
            }
        },
        error: function (jqXHR, textStatus, errorThrown) {
            console.error("Error from API", jqXHR);
...
class SearchableDropdownEditor {
    init(params) {
        ...
        // Create the container for the input and dropdown
        this.eGui = document.createElement('div');
        this.eGui.innerHTML = `
            <input type="text" class="search-input" placeholder="Search..." style="width: 100%; padding: 5px; margin-bottom: 5px;" />
            <select class="dropdown" style="width: 100%; padding: 5px;"></select>
        `;
        ...
        this.populateDropdown(params.values);

        // Add search filter functionality
        this.searchInput.addEventListener('input', () => {
            const filter = this.searchInput.value.toLowerCase();
            const filteredValues = params.values.filter(option =>
                option.toLowerCase().includes(filter)
            );
    ...
    isPopup() {
        return true; // Makes the dropdown a popup
    }
}
    ...
    const keys = Object.keys(anObject);
      keys.forEach(key => {
          ...
          if (dataType == "int" || dataType == "bigint" || dataType == "float" || dataType == "decimal" || dataType == "money") {
            ...
            // if (colName === 'sport') {
            //           cellEditorParams = { values: ["Swimming", "Gymnastics", "Cycling","Ski Jumping"] }; // Dropdown options for Name
            // }
            if(editData.length > 0){
              ...
              // if(1){  //to checck if field is drppdown
              let cellEditorParams = {};
              console.log("columns name===>", colName);

                  if (colName === 'Sport') {
                      (async () => {
                          // Function to fetch sports options dynamically
                          async function fetchSportOptions() {
                              try {
                                  const response = await fetch('sport_options.php'); // Replace with your PHP script's path
                                  const data = await response.json();
                                  ...
                                  if (Array.isArray(data)) {
                                      console.log("Fetched sports:", data);
                                      return data; // Return the fetched sports data
                                  } else {
                                      console.error("Failed to fetch sports:", data);
                                      return [];
                                  }
                          ...
                          // Fetch the sports options
                          const sportsOptions = await fetchSportOptions();

                          // Add the column definition for "Sport"
                          colDefs.push({
                              field: colName,
                              colId: colName,
                              cellDataType: 'text',
                              ...
                              cellEditor: MultiSelectDropdownEditor, // Use custom dropdown editor
                              cellEditorParams: {
                                  values: sportsOptions, // Use fetched sports data
                    ...
                    console.log("In country sections");
                      (async () => {
                          async function fetchCountryOptions() {
                              try {
                                  const response = await fetch('country_options.php'); // Replace with your PHP script's path
                                  ...
                                  if (Array.isArray(data)) {
                                      ...
                                      return data; // Return the fetched country data
                                  } else {
                          ...
                          colDefs.push({
                              ...
                              filterParams: filterParams,
                              editable: true, // Make the column editable
                              cellEditor: SearchableDropdownEditor, // Use custom dropdown editor
                              cellEditorParams: {
                                  values: countryOptions, // Use fetched country data
                                  valueListMaxHeight: 120, // Adjust dropdown height
                                  searchEnabled: true, // Enable search bar
                                  allowTyping: true,
                                  filterList: true,
