...
"kreport_datagrid_data","kreport_pivot_data","kReportFields","kReportFielters","kReportHeader","master_fieldtype","Master_ModuleType","pageroleaccesss","pagerolemaster","users");
	...
	/*	$metaConfig = array(
		...
		$result = db::getInstanceMaster()->db_select($sql, "M-130");
		...
		for($i = 0; $i < $result['num_rows']; $i++){ //WHILE LOOP FOR $row
			...
			$viewSettings[17] = $result['result_set'][$i]['formtype'];

			$viewSettings[18]  = $result['result_set'][$i]['ReportDefaultFromDate'];

			$viewSettings[19] = $result['result_set'][$i]['isOpenForDevelopment'];

			$viewSettings[20]  = $result['result_set'][$i]['ReportDefaultToDate'];
			...
			$db[5]  = $result['result_set'][$i]['BarcodeSPName'];
		...
		$result = db::getInstanceMaster()->db_select($sql, "M-274");
		...
		for($i = 0; $i < $result['num_rows']; $i++){ //WHILE LOOP FOR $row
				...
				$code[$i][45]  = trim($result['result_set'][$i]['TableFromDb']);
				...
				$code[$i][52]  = $result['result_set'][$i]['DependentField'];
				$code[$i][53]  = $result['result_set'][$i]['NonEditableFields'];
			...
			if($fld == 14 && $GridId > 0){
				...
				$gridinfo = array($code[$i][0],$code[$i][6],$code[$i][39],$code[$i][2],$code[$i][42], $code[$i][53]); //Added by pornima for showingMultiple select dropdown on 28/5/2024
				...
				*/
				...
				$result1 = db::getInstanceMaster()->db_select($sql1, "M-413");
				...
				for($j = 0; $j < $result1['num_rows']; $j++){ 
					...
					$moredb = array($result1['result_set'][$j]['TableName'],	//0
									...
									$result1['result_set'][$j]['RowwiseBarcodeUniqueID']
				...
				$moreResult = db::getInstanceMaster()->db_select($sql, "M-462");
				...
				for($j = 0; $j < $moreResult['num_rows']; $j++){ 
					...
					$more[$j][1] = trim($moreResult['result_set'][$j]['DbFieldName']);            //Db Field Name
					...
					$more[$j][6] = trim($moreResult['result_set'][$j]['FieldOtherConditions']);   //Other Conditions in an Input Tag
					...
					$more[$j][30] = $moreResult['result_set'][$j]['DependentField'];			//DependentField using clear dependent fields

					$more[$j][31] = $moreResult['result_set'][$j]['NonEditableFields'];			//NonEditableFields using for dependent field non editable

					$more[$j][32] = $moreResult['result_set'][$j]['ClickableInput'];			//ClickableInput using for grid field clickable
					...
					if($valueFromDB < 0){
						...
						$moreextradb[$GridId][$more[$j][1]][0] = $moreResult['result_set'][$j]['TableFromDb'];

						$moreextradb[$GridId][$more[$j][1]][1] = $moreResult['result_set'][$j]['TablePrimary'];

						$moreextradb[$GridId][$more[$j][1]][2] = $moreResult['result_set'][$j]['TableLabel'];

						$moreextradb[$GridId][$more[$j][1]][3] = $moreResult['result_set'][$j]['TableCondition'];

						$moreextradb[$GridId][$more[$j][1]][4] = "";

						$moreextradb[$GridId][$more[$j][1]][5] = $moreResult['result_set'][$j]['TableMapTable'];

						$moreextradb[$GridId][$more[$j][1]][6] = $moreResult['result_set'][$j]['TableMapPrimary'];

						$moreextradb[$GridId][$more[$j][1]][7] = $moreResult['result_set'][$j]['TableMapOtherKey'];
