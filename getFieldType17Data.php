        ...
        foreach ($DbFieldNames as $key => $value) {
            ...
            if (gettype($row[$i][$fieldName]) == 'object') {
                ...
                if ($fieldName == 'pending') {
                    ...
                    $sub_array[$fieldName] = '<input type="number" readonly class="form-control " name="pending" id="pending" style="text-align:right;" value="'.Encoding::fixUTF8($pending, Encoding::ICONV_IGNORE).'"/>';
            ...
            foreach ($finalResponseParams as $l => $param) {
                ...
                if (array_key_exists($paramLower, $rowLower)) {
                    ...
                    if ($paramLower == 'pending') {
                        // $dataArray[$param] = '<input type="number" readonly class="form-control" name="pending" id="pending" style="text-align:right;" value="'.Encoding::fixUTF8($row['pending'], Encoding::ICONV_IGNORE).'"/>';
