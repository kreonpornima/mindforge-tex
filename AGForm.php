...
<section class="panel" style="margin-bottom: -10px;">
    <header class="panel-heading" style="display: flex; justify-content: space-between; align-items: center;padding:10px;border-radius:5px;margin-bottom:5px;">
    <h2 class="panel-title"></h2>
    <div style="display: flex; align-items: center; gap: 10px;">
      <button class="btn btn-warning btn-xs" onclick="openLogModal('log')">LOG</button>
      <?php if($_SESSION['Category'] == 2){ ?>
        <button class="btn btn-info btn-xs" onclick="openSQLModal('full')">SQL</button>
        <button class="btn btn-success btn-xs" onclick="openRequirementModal()">Requirements</button>
      <?php } ?>
    </div>
  </header>
...
function replaceDdIdWithDdVal(data) {
    ...
    console.log("replaceDdIdWithDdVal fn")
...
function setRowData(newData) {
  ...
  const sanitizedData = expectedFields.reduce((obj, key) => {
    if (key.includes('_ddVal')) {
      obj[key] = key in newData ? newData[key] : '-';
    } else {
      obj[key] = key in newData ? newData[key] : null;
    }
    return obj;
  }, {});

  console.log('sanitizedData', expectedFields, sanitizedData, newData)
...
// expectedFields = gridOptions.columnDefs.map(def => def.field);
  ...
  function dynamicallyConfigureColumnsFromObject(anObject, editableStructure = []){
    ...
    const keys = Object.keys(anObject);
      keys.forEach(key => {
          ...
          expectedFields = [colName, ...expectedFields];
          ...
          if (+dataType == 5 || +dataType == 19 || +dataType == 17){
                          expectedFields = [colName + "_ddVal", ...expectedFields];
                          ...
                          // const countryOptions = await fetchDropDownDataOptions(FormID);
                           colDefs.push({
                              ...
                              suppressKeyboardEvent: (params) => {
                                ...
                                // Allow space key for cell editor
                                if (params.editing && key === ' ') {
                                  return true; // prevent grid from processing, let editor handle
                                }
                                return false; // grid can process other keys as normal
                              },
