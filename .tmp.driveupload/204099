...
"kreport_datagrid_data","kreport_pivot_data","kReportFields","kReportFielters","kReportHeader","master_fieldtype","Master_ModuleType","pageroleaccesss","pagerolemaster","users");
	...
	/*	$metaConfig = array(
		...
		for($i = 0; $i < $result['num_rows']; $i++){ //WHILE LOOP FOR $row
			...
			$editBtn = $delBtn = $otherBtn = $viewBtn = $saveAddMore = $saveAndPreview = 0;
		...
		for($i = 0; $i < $result['num_rows']; $i++){ //WHILE LOOP FOR $row
				...
				$code[$i][44]  = $result['result_set'][$i]['TableFromDb'];
				$code[$i][45]  = $result['result_set'][$i]['TablePrimary'];
				$code[$i][46]  = $result['result_set'][$i]['TableLabel'];
				$code[$i][47]  = $result['result_set'][$i]['TableCondition'];
				$code[$i][48]  = $result['result_set'][$i]['TableMapTable'];
				$code[$i][49]  = $result['result_set'][$i]['TableMapPrimary'];
				$code[$i][50]  = $result['result_set'][$i]['TableMapOtherKey'];
				...
				*/
				...
				for($j = 0; $j < $result1['num_rows']; $j++){ 
					...
					$moredb = array($result1['result_set'][$j]['TableName'],
									...
									$result1['result_set'][$j]['GridId'],  //Added by pornima for adding GridId in grid main div

									$result1['result_set'][$j]['GridImportSPName'],  //Added by Mitesh on 26/9/24 for importing bulk in grid using excel or csv

									$result1['result_set'][$j]['AfterGridAddCursorPosition'],  //AfterGridAddCursorPosition use where FieldType is 14 for after adding row in grid then cursor set on perticular field

									$result1['result_set'][$j]['OrderBy'],  //use for arrange order of grid data
				...
				for($j = 0; $j < $moreResult['num_rows']; $j++){ 
					...
					$more[$j][29] = $moreResult['result_set'][$j]['OtherAttributes'];			//OtherAttributes
...
*/
// echo "<pre>Moreextradb<br />"; print_r($moreextradb); echo "<br />";
// echo "DB<br />"; print_r($db); echo "<br />";
// echo "code<br />"; print_r($code); echo "<br />";
// echo "extradb<br />"; print_r($extradb); echo "<br />";
// echo "viewcode<br />"; print_r($viewcode); echo "<br />";
// echo "dynamix<br />"; print_r($dynamix); echo "<br />";
// echo "more<br />"; print_r($more); echo "<br />";
// echo "moredb<br />"; print_r($moredb); echo "<br />";
