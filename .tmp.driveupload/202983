...
$gridRowEditID = $_REQUEST['gridRowEditID'];
$barcodeFont = "Gloock";
...
for($i = 0; $i < sizeof($result['result_set'][0]); $i++){
    ...
    // if($FormID == 5327) {echo '<pre>'; print_r($row);}
    if(isset($row['barcodeFont'])){
        if(strlen($row['barcodeFont']) > 0){
            $barcodeFont = $row['barcodeFont'];
        }
    }
    echo '
        ...
        <link href="https://fonts.googleapis.com/css2?family='.$barcodeFont.'&display=swap" rel="stylesheet">
        <style>
            * {
                font-family: "'.$barcodeFont.'", serif;
                font-weight: 600;
                /*font-style: normal;*/
            }
            .barcode-wrapper {
