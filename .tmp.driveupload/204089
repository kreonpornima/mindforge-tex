...
function isPinnedRowDataCompleted(params) {
  ...
  function loadPivotGridData() {
      ...
      if(allData !== null){
        ...
        fetch("getKmainData.php?FormID=<?php echo $FormID; ?>")
...
//     // If the date is an object with a 'date' property, extract the string
//     if (typeof dateAsString === 'object' && dateAsString.date) {
//         dateAsString = dateAsString.date;
//     }

//     // If the date includes a time component, split it and take only the date part
//     if (typeof dateAsString === 'string' && dateAsString.includes(" ")) {
//         dateAsString = dateAsString.split(" ")[0]; // Keep only the date (YYYY-MM-DD)
//     }
...
//     // If it's a full Date object, convert it to YYYY-MM-DD format
//     if (dateAsString instanceof Date) {
//         return dateAsString.toISOString().split("T")[0]; // Extract YYYY-MM-DD
//     }

//     // If it's a string containing a timestamp, split and keep only the date part
//     if (typeof dateAsString === 'string' && dateAsString.includes(" ")) {
//         dateAsString = dateAsString.split(" ")[0]; // Keep only YYYY-MM-DD
//     }

//     // Handle date format (e.g., DD-MM-YYYY -> YYYY-MM-DD)
//     if (typeof dateAsString === 'string' && dateAsString.includes('-')) {
...
function UpdatedField(ID, Field, oldValue, newValue, params) {
    var FormID = <?php echo $FormID; ?>;
    console.log("FormID:", FormID); // Debug the FormID value
    console.log(`Old Value: ${oldValue}, New Value: ${newValue} for ID: ${ID} on Field: ${Field}`);
...
class ColourCellRenderer {
  ...
  function dynamicallyConfigureColumnsFromObject(anObject, editableStructure = []){
    var FormID = <?php echo $FormID; ?>;
    console.log("FormID:=>", FormID); // Debug the FormID value
    console.log("In dynamically method");
    ...
    const keys = Object.keys(anObject);
      keys.forEach(key => {
          ...
          if (+dataType == 7) {
            ...
            colDefs.push({ 'field': colName, 'colId':colName , 'cellRenderer': CompanyLogoRenderer });
          }else if (+dataType == 5 || +dataType == 19 || +dataType == 17){
                          ...
                          // const countryOptions = await fetchDropDownDataOptions(FormID);
                           colDefs.push({
                              ...
                              valueFormatter: function (params) {
                                ...
                                if(params.data[params.colDef.field] == params.value)
                                  return params.data[params.colDef.field + '_ddVal'];
                                else
                                  return params.value;
                              },
                      // valueFormatter: (params) => params.value.Label,
                      // valueParser: (params) => colors.find((color) => color.name === params.newValue),
                      // keyCreator: (params) => params.value.Label,
                              ...
                              // components: {
                              //     colourCellRenderer: ColourCellRenderer  // Register your custom renderer
                              // },
                              // cellRenderer: (params) => { 
                              //   console.log(" cellRenderer  Field Type 5 params Data... ", params);
                              //   // console.log("RENDERING... ", params.data.gold)
