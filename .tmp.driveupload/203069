...
$sql = "select r.*,m.* from requirements r LEFT JOIN kmainforms f ON r.FormId = f.FormId 
        LEFT JOIN Master_ModuleType m ON f.ModuleType = m.ID WHERE r.ID = ". $TaskID;
...
if($res['num_rows'] > 0){
    ...
    $sql = "UPDATE users SET LastCompanyID = ".$companyID.", LastDivisionID = ".$divisionID.", LastYear = '".$yearID."' WHERE ID = " . $_SESSION['user_id'];
    ...
    $query = "SELECT CONCAT(AiCompany.Label,' - ', AiDivision.Label, ' - ', AiReg.Year) as Name, Aireg.CompanyID, Aireg.DivisionID, Aireg.Year, Aireg.Database_name, Aireg.Ip, Aireg.Port, Aireg.Sql_User_Id, 
        ...
        where Aireg.CompanyID='".$companyID."' and Aireg.DivisionID='".$divisionID."' and Aireg.Year='".$yearID."' ";
...
// echo $URL . $FormID;
if(strlen($URLFrom) == 0) $URLFrom = $URL . $FormID;
?>
<script>window.location.replace('<?php echo $URLFrom;?>');</script>
